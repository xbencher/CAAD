...
TEST_ENV := ENV=test \
	...
	JWT_SECRET=test-secret-please-change-1234567 \
	OTP_PEPPER=test-otp-pepper-please-change-me \
	PHONE_HASH_PEPPER=test-phone-hash-pepper-change
...
test-backend:
	...
	cd backend && $(TEST_ENV) uv run pytest --cov=app --cov-report=term-missing
